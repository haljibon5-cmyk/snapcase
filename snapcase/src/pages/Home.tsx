        ...
        if (data && data.length > 0) {
          ...
          if (settings && settings.description) {
            try {
              ...
              if (
                parsed.bannerImages &&
                Array.isArray(parsed.bannerImages) &&
                parsed.bannerImages.length > 0
              ) {
                setBannerImages(parsed.bannerImages);
              }
              if (
                parsed.bottomBannerImages &&
                Array.isArray(parsed.bottomBannerImages) &&
                parsed.bottomBannerImages.length > 0
              ) {
              ...
              if (
                ...
                Array.isArray(parsed.preTestimonialBannerImages) &&
                parsed.preTestimonialBannerImages.length > 0
              ) {
  ...
  if (isLoading) {
    return (
      <div className="min-h-screen flex items-center justify-center bg-[#faf9f6]">
        <div className="w-10 h-10 border-4 border-gray-200 border-t-[#86bfbf] rounded-full animate-spin"></div>
      </div>
    );
  }
    ...
    <div className="min-h-screen flex flex-col bg-[#faf9f6]">
            ...
            <div className="flex flex-col lg:flex-row gap-6 lg:gap-16">
              ...
              <div className="w-full lg:w-1/2 grid grid-cols-2 gap-x-6 gap-y-8 md:gap-x-12 md:gap-y-12 content-center">
                {[0, 1, 2, 3]
                  .map(
                    (offset) =>
                      featuredProducts[offset % featuredProducts.length],
                  )
                  .map((product, idx) => (
      ...
      <section className="w-full my-6 md:my-10 cursor-pointer hover:opacity-95 transition-opacity overflow-hidden">
        <Link to="/products" className="grid grid-cols-5 w-full h-full">
          {bannerImages.map((img, idx) => (
            <div
              key={idx}
              className="aspect-[3/4] md:aspect-[4/5] overflow-hidden relative"
            >
              <img
                src={img}
      ...
      <section className="py-12 md:py-20 px-4 md:px-8 bg-white w-full max-w-6xl mx-auto">
        <div className="flex flex-col items-center text-center mb-8 md:mb-12">
          <h2 className="text-3xl font-serif tracking-wide text-gray-900 mb-2">
            SHOP IG
          </h2>
          <a
            href={igLink}
      ...
      <section className="py-16 bg-white px-6 md:px-12 w-full">
        <div className="max-w-6xl mx-auto">
          <div className="flex flex-col items-center text-center mb-16">
            <h2 className="text-3xl font-serif tracking-wide text-gray-900 mb-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-12">
            {[
              {
                text: "Absolutely stunning craftsmanship. I get compliments every time I use my new phone case.",
                author: "Sarah M.",
              },
              {
                text: "Beautiful accessories that truly stand out from the crowd. The attention to detail is incredible.",
                author: "Jessica T.",
              },
              {
                text: "So unique and elegant. I've bought three different items and love them all!",
                author: "Amanda R.",
              },
            ].map((review, i) => (
              <div
                key={i}
                className="flex flex-col items-center text-center p-8 bg-[#faf9f6]"
              >
                ...
                <p className="text-gray-600 italic mb-8 flex-grow tracking-wide leading-relaxed">
                  "{review.text}"
                </p>
